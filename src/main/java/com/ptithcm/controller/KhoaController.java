...
public class KhoaController {
	...
	private void populateCanDelete(Session session, List<Khoa> list) {
		...
		List<String> khoaWithGV = session.createQuery("SELECT distinct trim(maKhoa) FROM GiangVien", String.class).list();
		List<String> khoaWithLTC = session.createQuery("SELECT distinct trim(maKhoa) FROM LopTinChi", String.class).list();
