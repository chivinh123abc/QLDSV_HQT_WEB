...
public class AccountController {
    ...
    public String index(ModelMap model, jakarta.servlet.http.HttpSession httpSession) {
        ...
        List<Object[]> gvList =
                session.createQuery("SELECT maGV, ho, ten FROM GiangVien", Object[].class).list();
        for (Object[] gv : gvList) {
            nameMap.put(
                    (String) gv[0],
                    (gv[1] != null ? gv[1] + " " : "") + (gv[2] != null ? gv[2] : ""));
        }
        List<Object[]> svList =
                session.createQuery("SELECT maSV, ho, ten FROM SinhVien", Object[].class).list();
        for (Object[] sv : svList) {
            nameMap.put(
                    (String) sv[0],
                    (sv[1] != null ? sv[1] + " " : "") + (sv[2] != null ? sv[2] : ""));
        }

        // Find lecturers with credit classes
        List<String> ltcMaGV =
                session.createQuery(
                                "SELECT distinct trim(maGV) FROM LopTinChi WHERE maGV IS NOT NULL",
                                String.class)
                        .list();
        ...
        List<String> dkMaSV =
                session.createQuery(
                                "SELECT distinct trim(maSV) FROM DangKy WHERE maSV IS NOT NULL",
                                String.class)
                        .list();
    ...
    public Map<String, Object> saveUser(
        ...
        try {
            // If username does not match selectedId, ensure it does not steal another existing
            // MaGV/MaSV
            if (user.getSelectedId() != null && !user.getUsername().equals(user.getSelectedId())) {
                Long countGV =
                        session.createQuery(
                                        "SELECT COUNT(*) FROM GiangVien WHERE maGV = :username",
                                        Long.class)
                                .setParameter("username", user.getUsername())
                                .uniqueResult();
                Long countSV =
                        session.createQuery(
                                        "SELECT COUNT(*) FROM SinhVien WHERE maSV = :username",
                                        Long.class)
                                .setParameter("username", user.getUsername())
                                .uniqueResult();
            ...
            if (mode.equals("add")) {
                ...
                if (existCount > 0) {
                    ...
                    res.put(
                            ...
                            "Tên đăng nhập ["
                                    + user.getUsername()
                                    + "] đã được cấp tài khoản trước đó!");
                    return res;
                }
                ...
                if (currentUser != null && currentUser.getUserId() == user.getUserId()) {
                    if (currentUser.getRoleId() != user.getRoleId()) {
                        ...
                        res.put(
                                "message",
                                "Không thể tự thay đổi Nhóm quyền của tài khoản đang đăng nhập!");
                        return res;
                    }
    ...
    public Map<String, Object> deleteUser(
            @RequestParam("userId") int userId, jakarta.servlet.http.HttpSession httpSession) {
        Map<String, Object> res = new HashMap<>();
        Session session = factory.openSession();
        org.hibernate.Transaction t = session.beginTransaction();
        try {
            ...
            if (user != null) {
                ...
                Long ltcCount =
                        session.createQuery(
                                        "SELECT COUNT(*) FROM LopTinChi WHERE upper(trim(maGV)) = upper(trim(:uname))",
                                        Long.class)
                ...
                if (ltcCount > 0) {
                    ...
                    res.put(
                            "message",
                            "Không thể xóa: Tài khoản này thuộc về Giảng viên đang phụ trách "
                                    + ltcCount
                                    + " lớp tín chỉ!");
                    return res;
                }
                ...
                Long dkCount =
                        session.createQuery(
                                        "SELECT COUNT(*) FROM DangKy WHERE upper(trim(maSV)) = upper(trim(:uname))",
                                        Long.class)
                                .setParameter("uname", uname)
                                .uniqueResult();
                if (dkCount > 0) {
                    ...
                    res.put(
                            "message",
                            "Không thể xóa: Tài khoản này thuộc về Sinh viên đã đăng ký "
                                    + dkCount
                                    + " lớp tín chỉ!");
                    return res;
                }
