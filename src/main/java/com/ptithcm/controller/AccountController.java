...
public class AccountController {
	...
	public String index(ModelMap model, jakarta.servlet.http.HttpSession httpSession) {
		...
		List<String> ltcMaGV = session.createQuery("SELECT distinct trim(maGV) FROM LopTinChi WHERE maGV IS NOT NULL", String.class).list();
		java.util.Set<String> dependentUsernames = new java.util.HashSet<>();
		for (String s : ltcMaGV) if (s != null) dependentUsernames.add(s.trim().toUpperCase());

		// Find students with registrations
		List<String> dkMaSV = session.createQuery("SELECT distinct trim(maSV) FROM DangKy WHERE maSV IS NOT NULL", String.class).list();
		for (String s : dkMaSV) if (s != null) dependentUsernames.add(s.trim().toUpperCase());
	...
	@ResponseBody
	public Map<String, Object> saveUser(@RequestBody Users user, @RequestParam("mode") String mode, jakarta.servlet.http.HttpSession httpSession) {
		Map<String, Object> res = new HashMap<>();
		Session session = factory.openSession();
		org.hibernate.Transaction t = session.beginTransaction();
		try {
			// If username does not match selectedId, ensure it does not steal another existing MaGV/MaSV
			if (user.getSelectedId() != null && !user.getUsername().equals(user.getSelectedId())) {
				...
				if (countGV > 0 || countSV > 0) {
					...
					res.put("message", "Tên đăng nhập [" + user.getUsername() + "] trùng với Mã của một Giảng viên/Sinh viên khác! Vui lòng đặt tên khác.");
					return res;
				}
			}
			...
			if (mode.equals("add")) {
				...
				Long existCount = session.createQuery("SELECT COUNT(*) FROM Users WHERE username = :username", Long.class)
						.setParameter("username", user.getUsername()).uniqueResult();
				if (existCount > 0) {
					res.put("status", "error");
				...
				Long existCount = session.createQuery("SELECT COUNT(*) FROM Users WHERE username = :username AND userId != :userId", Long.class)
						.setParameter("username", user.getUsername())
						.setParameter("userId", user.getUserId()).uniqueResult();
				if (existCount > 0) {
					res.put("status", "error");
					res.put("message", "Tên đăng nhập [" + user.getUsername() + "] đã được sử dụng bởi tài khoản khác!");
					return res;
				}
	...
	@ResponseBody
	public Map<String, Object> deleteUser(@RequestParam("userId") int userId, jakarta.servlet.http.HttpSession httpSession) {
		Map<String, Object> res = new HashMap<>();
		Session session = factory.openSession();
		org.hibernate.Transaction t = session.beginTransaction();
		try {
			...
			if (user != null) {
				...
				Long ltcCount = session.createQuery("SELECT COUNT(*) FROM LopTinChi WHERE upper(trim(maGV)) = upper(trim(:uname))", Long.class)
						.setParameter("uname", uname).uniqueResult();
				if (ltcCount > 0) {
					res.put("status", "error");
					res.put("message", "Không thể xóa: Tài khoản này thuộc về Giảng viên đang phụ trách " + ltcCount + " lớp tín chỉ!");
					return res;
				}
				
				// Check if this user is a student with registrations
				Long dkCount = session.createQuery("SELECT COUNT(*) FROM DangKy WHERE upper(trim(maSV)) = upper(trim(:uname))", Long.class)
						.setParameter("uname", uname).uniqueResult();
				if (dkCount > 0) {
					res.put("status", "error");
					res.put("message", "Không thể xóa: Tài khoản này thuộc về Sinh viên đã đăng ký " + dkCount + " lớp tín chỉ!");
					return res;
				}
