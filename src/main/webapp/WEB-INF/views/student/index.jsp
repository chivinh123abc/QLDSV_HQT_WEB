...
<head>
    ...
    <style>
        ...
        .table-clickable tbody tr { cursor: pointer; transition: all 0.2s; }
        .table-clickable tbody tr:hover { background-color: #eff6ff !important; transform: translateX(4px); }
        .student-title { font-weight: 700; color: #1e293b; margin-bottom: 2px; }
        .student-subtitle { font-size: 0.8rem; color: #94a3b8; display: flex; align-items: center; gap: 4px; }
...
<body>
                        ...
                        <div class="card-body px-4 pb-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <div class="input-group" style="max-width: 300px;">
                                    ...
                                    <input type="text" class="form-control bg-light border-0 small" placeholder="Tìm mã lớp hoặc tên lớp...">
                                </div>
                                <div class="d-flex gap-2">
                                    <select id="khoa-filter" class="form-select form-select-sm border-0 bg-light text-muted fw-bold" onchange="filterByKhoa()">
                                ...
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody id="class-table-body" class="border-top-0">
                                        <c:forEach var="lop" items="${lopList}">
                                            <tr onclick="window.location.href='${pageContext.request.contextPath}/student?maLop=${lop.maLop}'" 
                            ...
                            <c:when test="${not empty maLop}">
                                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                                    <div class="card-header bg-white border-bottom-0 pt-4 px-4 pb-3 d-flex justify-content-between align-items-center">
                                        ...
                                        <button class="btn btn-primary btn-sm rounded-3 px-3 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#studentModal" onclick="resetForm()">
                                    ...
                                    <div class="card-body px-4 pb-4">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <div class="input-group" style="max-width: 300px;">
                                                <span class="input-group-text bg-light border-0"><i class="bi bi-search text-muted"></i></span>
                                                <input type="text" class="form-control bg-light border-0 small" placeholder="Tìm MSV hoặc tên sinh viên...">
                                            </div>
                                        </div>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div class="fw-bold text-dark d-flex align-items-center gap-2">
                                                                    <i class="bi bi-person-circle text-muted"></i> ${sv.ho} ${sv.ten}
                                                                </div>
                                                            </td>
                                                            <td class="text-center">
                                                                <span class="badge rounded-pill border border-secondary text-secondary px-3 py-1 small">
                                                                    <i class="bi ${sv.phai == 'Nam' ? 'bi-gender-male' : 'bi-gender-female'} me-1"></i> ${sv.phai}
                                                                </span>
                                                            </td>
                                                            <td class="text-center text-muted small">
                                                                <i class="bi bi-calendar3 me-1"></i> <fmt:formatDate value="${sv.ngaySinh}" pattern="dd/MM/yyyy" />
                                                            </td>
                                                            <td class="text-muted small"><i class="bi bi-geo-alt me-1"></i> ${sv.diaChi}</td>
                                                            <td class="text-center">
                                                                <c:choose>
                                                                    <c:when test="${sv.dangNghiHoc}">
                                                                        <span class="badge border border-danger text-danger rounded-pill px-3 py-1 small fw-bold">
                                                                            <i class="bi bi-dot"></i> Đang nghỉ
                                                                        </span>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        <span class="badge border border-success text-success rounded-pill px-3 py-1 small fw-bold">
                                                                            <i class="bi bi-dot"></i> Đang học
                                                                        </span>
                                                                    </c:otherwise>
                                                                </c:choose>
                                                            </td>
                                                            <td class="text-center">
                                                                <div class="d-flex gap-2 justify-content-center">
                                                                    <button type="button" onclick="selectStudent('${sv.maSV}', 'edit')" class="btn btn-sm btn-outline-primary border-0 rounded-3">
                                                                        <i class="bi bi-pencil-square"></i>
                                                                    </button>
                                                                    <button type="button" onclick="selectStudent('${sv.maSV}', 'delete')" class="btn btn-sm btn-outline-danger border-0 rounded-3">
                                        ...
                                        <div class="mt-3 d-flex justify-content-between align-items-center">
                                            <div class="text-muted small">Hiển thị <strong>${sinhVienList.size()}</strong> sinh viên</div>
                                            <nav>
                ...
                <form id="studentForm" action="${pageContext.request.contextPath}/student" method="post">
                    <div class="modal-body p-4">
                        ...
                        <div class="d-flex gap-2 mb-4 p-2 bg-light rounded-3 border shadow-sm">
                            ...
                            <button type="submit" id="real_submit_insert" name="btnInsert" style="display:none;"></button>
                            <button type="submit" id="real_submit_update" name="btnUpdate" style="display:none;"></button>
                            <button type="submit" id="real_submit_delete" name="btnDelete" style="display:none;"></button>
                        </div>

                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-muted">MÃ SINH VIÊN</label>
                                <input type="text" class="form-control rounded-3" id="inp_maSV" name="maSV" value="${sinhVien.maSV}" placeholder="VD: N23DCCN001" required disabled>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-muted">HỌ</label>
                                <input type="text" class="form-control rounded-3" id="inp_ho" name="ho" value="${sinhVien.ho}" placeholder="VD: Nguyễn Văn" required disabled>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-muted">TÊN</label>
                                <input type="text" class="form-control rounded-3" id="inp_ten" name="ten" value="${sinhVien.ten}" placeholder="VD: An" required disabled>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="date" class="form-control rounded-3" id="inp_ngaySinh" name="ngaySinh" value="${fmtDate}" required disabled>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-muted">LỚP HIỆN TẠI</label>
                                <input type="text" class="form-control rounded-3 bg-light" name="maLop" value="${not empty maLop ? maLop : sinhVien.maLop}" readonly>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-muted">ĐỊA CHỈ</label>
                                <input type="text" class="form-control rounded-3" id="inp_diaChi" name="diaChi" value="${sinhVien.diaChi}" placeholder="VD: 97 Man Thiện, Q.9, TP.HCM" disabled>
                            </div>
                            <div class="col-md-12">
                                <div class="form-check form-switch p-0 d-flex align-items-center gap-2">
                                    <input class="form-check-input ms-0 mt-0" type="checkbox" role="switch" id="dangNghiHoc" name="dangNghiHoc" value="true" ${sinhVien.dangNghiHoc ? 'checked' : ''} disabled>
                                    <label class="form-check-label fw-bold text-danger" for="dangNghiHoc">Đang nghỉ học</label>
                                </div>
                            </div>
                        </div>
                                ...
                                <table class="table table-hover table-sm align-middle mb-0">
                                    ...
                                    <tbody id="mini-table-body">
                                        <c:forEach var="sv_item" items="${sinhVienList}">
                                            <tr class="${sv_item.maSV == sinhVien.maSV ? 'table-primary shadow-sm' : ''}">
                                                <td class="px-3"><span class="badge bg-primary bg-opacity-10 text-primary small">${sv_item.maSV}</span></td>
                                                <td class="fw-bold text-dark small">${sv_item.ho} ${sv_item.ten}</td>
                                                <td class="text-center small"><span class="badge border text-secondary rounded-pill px-2">${sv_item.phai}</span></td>
                                                <td class="text-center">
                                                    <div class="d-flex justify-content-center gap-1">
                                                        <button type="button" onclick="selectStudent('${sv_item.maSV}', 'edit')" class="btn btn-xs btn-outline-primary border-0 p-1">
                                                            <i class="bi bi-pencil-square"></i>
                                                        </button>
                                                        <button type="button" onclick="selectStudent('${sv_item.maSV}', 'delete')" class="btn btn-xs btn-outline-danger border-0 p-1">
                                                            <i class="bi bi-trash3"></i>
                                                        </button>
                                                    </div>
                                                </td>
                                        ...
                                        <c:if test="${empty sinhVienList}">
                                            <tr><td colspan="4" class="text-center py-3 text-muted small">Chưa có sinh viên</td></tr>
                                        </c:if>
                                    </tbody>
                                </table>
                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-body text-center p-4">
                    ...
                    <button type="button" class="btn btn-primary w-100 rounded-3 fw-bold shadow-sm" data-bs-dismiss="modal">ĐÓNG</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function renderTable(containerId, students, isMini) {
            ...
            container.innerHTML = students.map(sv => {
                ...
                const actionBtns = '<div class="d-flex justify-content-center gap-1">' +
                    '<button type="button" onclick="selectStudent(\'' + sv.maSV + '\', \'edit\')" class="btn ' + (isMini ? 'btn-xs' : 'btn-sm') + ' btn-outline-primary border-0">' +
                        '<i class="bi bi-pencil-square"></i>' +
                    '</button>' +
                    '<button type="button" onclick="selectStudent(\'' + sv.maSV + '\', \'delete\')" class="btn ' + (isMini ? 'btn-xs' : 'btn-sm') + ' btn-outline-danger border-0">' +
                        '<i class="bi bi-trash3"></i>' +
                    '</button>' +
                '</div>';
        ...
        async function selectStudent(maSV, mode) {
            try {
        ...
        async function performSave() {
            ...
            try {
                const response = await fetch(contextPath + '/student/api/save', {
                    method: 'POST',
        ...
        function renderClassTable(classes) {
            ...
            container.innerHTML = classes.map(lop => {
                ...
                return `
                    <tr onclick="window.location.href='${contextPath}/student?maLop=\${lop.maLop}'" 
                        style="cursor: pointer; transition: 0.2s;" 
                        class="\${isActive ? 'table-primary shadow-sm' : ''} class-row">
                        <td class="px-3">
                            <span class="badge \${isActive ? 'bg-primary' : 'bg-primary bg-opacity-10 text-primary'} rounded-2 px-3 py-2">
                                \${lop.maLop}
                            </span>
                        </td>
                        <td class="fw-semibold text-dark">\${lop.tenLop}</td>
