                                        ...
                                        <table class="table table-custom align-middle mb-0">
                                            ...
                                            <tbody id="subject-table-body">
                                                <c:forEach var="item" items="${monHocList}">
                                                    <tr <c:if test="${sessionScope.role == 'PGV'}">onclick="selectSubject('${item.maMH}', 'edit', false)"</c:if>>
                                                        <td class="px-3">
                                                        ...
                                                        <c:if test="${sessionScope.role == 'PGV'}">
                                                            <td class="text-center">
                                                                <div class="d-flex gap-2 justify-content-center">
                                                                    <button
                                                                        onclick="event.stopPropagation(); selectSubject('${item.maMH}', 'edit', true)"
                                    ...
                                    <table class="table table-hover table-sm align-middle mb-0">
                                        ...
                                        <tbody id="mini-table-body">
                                            <c:forEach var="m" items="${monHocList}">
                                                <tr <c:if test="${sessionScope.role == 'PGV'}">onclick="selectSubject('${m.maMH}', 'edit', false)"</c:if>>
                                                    <td class="px-3"><span
                                                            class="badge bg-primary bg-opacity-10 text-primary small">${m.maMH}</span>
                                                    </td>
                                                    <td class="fw-bold text-dark small">${m.tenMH}</td>
                                                    <td class="text-center small text-muted">${m.soTietLT}/${m.soTietTH}
                                                    </td>
                                                    <td class="text-center">
                                                        <div class="d-flex justify-content-center gap-1">
                                                            <button type="button"
                                                                onclick="event.stopPropagation(); selectSubject('${m.maMH}', 'edit', true)"
                    ...
                    <td class="text-center">
                        <div class="d-flex gap-2 justify-content-center">
                            <button onclick="event.stopPropagation(); selectSubject('\${item.maMH}', 'edit', true)" class="btn btn-sm btn-outline-primary border-0 rounded-3"><i class="bi bi-pencil-square"></i></button>
                             <button onclick="event.stopPropagation(); selectSubject('\${item.maMH}', 'delete', true)" class="btn btn-sm btn-outline-danger border-0 rounded-3 \${!item.canDelete ? 'disabled opacity-25' : ''}" \${!item.canDelete ? 'disabled title="Không thể xóa"' : ''}><i class="bi bi-trash3"></i></button>
                         </div>
                     </td>
                 </tr>
            `).join('');
                    ...
                    <td class="text-center">
                        <div class="d-flex justify-content-center gap-1">
                            <button type="button" onclick="event.stopPropagation(); selectSubject('\${item.maMH}', 'edit', true)" class="btn btn-xs btn-outline-primary border-0 p-1">
                             ...
                             <button type="button" onclick="event.stopPropagation(); selectSubject('\${item.maMH}', 'delete', true)" class="btn btn-xs btn-outline-danger border-0 p-1 \${!item.canDelete ? 'disabled opacity-25' : ''}" \${!item.canDelete ? 'disabled title="Không thể xóa"' : ''}>
                                 <i class="bi bi-trash3"></i>
                             </button>
                         </div>
                     </td>
