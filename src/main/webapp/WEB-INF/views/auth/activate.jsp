...
<head>
    ...
    <title><s:message code="auth.activate.title"/> - QLDSV PTITHCM</title>
    <!-- Google Fonts -->
...
<body>
    <div class="login-card animate__animated animate__zoomIn">
        <div class="login-header">
            ...
            <c:choose>
                <c:when test="${empty STEP_OTP_SENT}">
                    <p class="text-muted"><s:message code="auth.activate.subtitle.step1"/></p>
                </c:when>
                <c:otherwise>
                    <p class="text-muted small"><s:message code="auth.activate.subtitle.step2"/> <br><strong>${maskedEmail}</strong></p>
                </c:otherwise>
            </c:choose>
        </div>
        ...
        <c:choose>
            <%-- STAGE 1: Request OTP --%>
            <c:when test="${empty STEP_OTP_SENT}">
                <form action="${pageContext.request.contextPath}/auth/activate/request-otp" method="post">
                    ...
                    <button type="submit" class="login-btn"><s:message code="auth.activate.btn.requestOtp"/></button>
                </form>

                <div class="mt-4 text-center">
                    <a href="${pageContext.request.contextPath}/login" class="text-decoration-none small fw-semibold text-muted">
                        <i class="bi bi-arrow-left me-1"></i><s:message code="auth.activate.link.backLogin"/>
                    </a>
                </div>
            </c:when>
            ...
            <c:otherwise>
                <form action="${pageContext.request.contextPath}/auth/activate/confirm" method="post">
                    <input type="hidden" name="csrf_token" value="${csrfToken}" />
                    ...
                    <button type="submit" class="login-btn"><s:message code="auth.activate.btn.confirm"/></button>
                </form>

                <div class="mt-4 d-flex justify-content-between align-items-center">
                    <a href="${pageContext.request.contextPath}/auth/activate/reset" class="text-decoration-none small fw-semibold text-muted">
                        <i class="bi bi-arrow-left me-1"></i><s:message code="auth.activate.link.reset"/>
                    </a>
                    <a href="${pageContext.request.contextPath}/auth/activate" class="text-decoration-none small fw-semibold text-primary">
                        <s:message code="auth.activate.link.resend"/> <i class="bi bi-arrow-right-short"></i>
                    </a>
                </div>
            </c:otherwise>
        </c:choose>
